...
mixin SceneLayers {
  ...
  void paintFarBackdrop(Canvas canvas, TreeFrame frame) {
    ...
    switch (spec.backdrop) {
      case Backdrop.river:
        ...
        canvas.drawRect(Rect.fromLTRB(0, top, w, groundTop), paint..color = palette.water ?? palette.far);
        final streak = Paint()
      ...
      case Backdrop.vineyard:
        ...
        for (var row = 0; row < 4; row++) {
          ...
          final offset = step * (row.isEven ? 0.25 : 0.6);
          final post = Paint()
            ..color = palette.groundDeep.withValues(alpha: 0.55)
            ..strokeWidth = math.max(0.7, w * 0.004 * perspective);
          final vine = Paint()..color = palette.farAlt.withValues(alpha: 0.9);
          final grape = Paint()..color = spec.accent.withValues(alpha: 0.9);
          var k = 0;
      ...
      case Backdrop.sea:
        ...
        canvas.drawRect(Rect.fromLTRB(0, horizon, w, groundTop), paint..color = palette.water ?? palette.far);
        ...
        for (var i = 0; i < 5; i++) {
          ...
          final count = 9 + i * 2;
          final period = w / count;
          final shift = still ? 0.0 : (timeMs * 0.012 * (1 + i * 0.2)) % period - period;
          swell.color = spec.accent.withValues(alpha: 0.2 + i * 0.07);
          canvas.drawPath(_wavePath(w, y, h * 0.006 * (1 + i * 0.3), count, shift), swell);
        }
        ...
        canvas.drawCircle(
          ...
          Paint()
            ..shader = ui.Gradient.radial(Offset(sx, sy), sr * 5, [
              spec.accent.withValues(alpha: 0.33),
    ...
    switch (spec.backdrop) {
      ...
      case Backdrop.river:
        ...
        final stalk = Paint()
          ...
          ..color = palette.groundDeep;
        final head = Paint()..color = spec.accent;
        for (var i = 0; i < 10; i++) {
      ...
      case Backdrop.sea:
        // Foam where the last wave reaches the sand, and two shells.
        final foam = Paint()
          ..style = PaintingStyle.stroke
        ...
        canvas.drawPath(_wavePath(w, groundTop + band * 0.04, band * 0.05, 11, shift), foam);
  ...
  void _drawFox(Canvas canvas, Offset p, double scale) {
    ...
    final x = p.dx;
    final y = p.dy;
    const body = Color(0xFFD2692E);
    const dark = Color(0xFF8E4A1E);
    const cream = Color(0xFFF4EDE2);
    final wag = still ? 0.0 : math.sin(timeMs * 0.002) * 0.08;
    final fill = Paint()..color = body;
    final pale = Paint()..color = cream;
    final ink = Paint()..color = dark;
    // The tail curls round the front.
    canvas.save();
    canvas.translate(x - s * 1.1, y - s * 0.45);
    canvas.rotate(-0.5 + wag);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 2.6, height: s * 1.0), fill);
    canvas.restore();
    canvas.drawCircle(Offset(x - s * 2.1, y - s * 0.85), s * 0.3, pale);
    // Body, chest.
    canvas.drawOval(Rect.fromCenter(center: Offset(x, y - s * 1.0), width: s * 1.7, height: s * 2.1), fill);
    canvas.drawOval(Rect.fromCenter(center: Offset(x + s * 0.25, y - s * 0.75), width: s * 0.8, height: s * 1.2), pale);
    // Head and ears.
    canvas.drawCircle(Offset(x + s * 0.35, y - s * 2.05), s * 0.55, fill);
    canvas.drawPath(
      Path()
    ...
    // Snout, nose, eye, paws.
    canvas.drawOval(Rect.fromCenter(center: Offset(x + s * 0.7, y - s * 1.95), width: s * 0.7, height: s * 0.44), pale);
    canvas.drawCircle(Offset(x + s * 0.98, y - s * 1.97), math.max(0.5, s * 0.08), ink);
    canvas.drawCircle(Offset(x + s * 0.5, y - s * 2.15), math.max(0.5, s * 0.07), ink);
    for (final px in const [0.35, 0.75]) {
      canvas.drawOval(Rect.fromCenter(center: Offset(x + s * px, y - s * 0.05), width: s * 0.44, height: s * 0.24), ink);
    }
  }
  ...
  void _drawDonkey(Canvas canvas, Offset p, double scale) {
    ...
    canvas.drawOval(Rect.fromCenter(center: Offset(x, y - s * 1.45), width: s * 3.0, height: s * 1.5), fill);
    // Neck, head, muzzle.
    canvas.drawLine(
    ...
    canvas.save();
    canvas.translate(x - s * 1.95, y - s * 2.7);
    canvas.rotate(0.35);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 1.1, height: s * 0.76), fill);
    canvas.restore();
    canvas.save();
    canvas.translate(x - s * 2.35, y - s * 2.55);
    canvas.rotate(0.35);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 0.6, height: s * 0.44), Paint()..color = light);
    canvas.restore();
    // Ears.
    canvas.save();
    canvas.translate(x - s * 1.75, y - s * 3.3);
    canvas.rotate(-0.25 - flick);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 0.28, height: s * 1.0), fill);
    canvas.restore();
    canvas.save();
    canvas.translate(x - s * 1.5, y - s * 3.2);
    canvas.rotate(0.15);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 0.28, height: s * 1.0), fill);
    canvas.restore();
    // Mane, eye, tail.
    ...
    for (var i = 0; i < 3; i++) {
      ...
      canvas.drawLine(Offset(mx, my), Offset(mx + s * 0.2, my - s * 0.12), ink);
    }
    final dot = Paint()..color = dark;
    canvas.drawCircle(Offset(x - s * 2.05, y - s * 2.8), math.max(0.5, s * 0.07), dot);
    canvas.drawLine(Offset(x + s * 1.4, y - s * 1.6), Offset(x + s * 1.75, y - s * 0.9), ink);
    canvas.drawCircle(Offset(x + s * 1.78, y - s * 0.82), s * 0.15, dot);
  }
  ...
  void _drawStork(Canvas canvas, Offset p, double scale) {
    ...
    final body = Paint()..color = white;
    canvas.save();
    canvas.translate(x + s * 0.1, y - s * 2.1);
    canvas.rotate(-0.15);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 1.8, height: s * 1.0), body);
    canvas.restore();
    canvas.save();
    canvas.translate(x + s * 0.7, y - s * 2.05);
    canvas.rotate(-0.2);
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 0.9, height: s * 0.56), Paint()..color = black);
    canvas.restore();
    // Neck and head.
    ...
    canvas.drawCircle(Offset(x - s * 0.9, hy), s * 0.3, body);
    canvas.drawPath(
      Path()
        ..moveTo(x - s * 1.1, hy)
  ...
  void _drawLion(Canvas canvas, Offset p, double scale) {
    ...
    final x = p.dx;
    final y = p.dy;
    const tan = Color(0xFFC9973F);
    const mane = Color(0xFF8E5E1E);
    const dark = Color(0xFF5C3A12);
    const cream = Color(0xFFEBD9A8);
    ...
    final maneFill = Paint()..color = mane;
    canvas.drawCircle(Offset(x + s * (2.45 + sway), y - s * 0.18), s * 0.2, maneFill);
    final fill = Paint()..color = tan;
    canvas.drawOval(Rect.fromCenter(center: Offset(x + s * 0.2, y - s * 0.75), width: s * 3.4, height: s * 1.4), fill);
    for (final (ox, oy) in const [(-1.0, -0.25), (-0.8, -0.1)]) {
      canvas.drawOval(Rect.fromCenter(center: Offset(x + s * ox, y + s * oy), width: s * 1.4, height: s * 0.42), fill);
    }
    canvas.drawCircle(Offset(x - s * 1.05, y - s * 1.5), s * 0.85, maneFill);
    canvas.drawCircle(Offset(x - s * 1.15, y - s * 1.45), s * 0.55, fill);
    for (final ex in const [-0.75, -1.45]) {
      canvas.drawCircle(Offset(x + s * ex, y - s * 1.95), s * 0.16, fill);
    }
    canvas.drawOval(Rect.fromCenter(center: Offset(x - s * 1.35, y - s * 1.3), width: s * 0.64, height: s * 0.44), Paint()..color = cream);
    final ink = Paint()..color = dark;
    canvas.drawCircle(Offset(x - s * 1.6, y - s * 1.38), math.max(0.5, s * 0.08), ink);
    for (final ex in const [-1.3, -1.0]) {
      canvas.drawCircle(Offset(x + s * ex, y - s * 1.6), math.max(0.5, s * 0.06), ink);
    }
  }
  ...
  void _drawBees(Canvas canvas, TreeFrame frame) {
    ...
    for (final bee in decor.bees) {
      ...
      final y = cy + math.sin(tt * 1.7 + p) * ry * bee.ry + (still ? 0.0 : math.sin(timeMs * 0.02 + p) * size * 0.3);
      for (final side in const [-1.0, 1.0]) {
        canvas.save();
        canvas.translate(x + side * size * 0.4, y - size * 0.7);
        canvas.rotate(side * -0.6);
        canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: size * 0.9, height: size * 0.6), wing);
        canvas.restore();
      }
      canvas.drawOval(Rect.fromCenter(center: Offset(x, y), width: size * 2, height: size * 1.3), body);
      canvas.drawRect(Rect.fromCenter(center: Offset(x, y), width: size * 0.36, height: size * 1.2), stripe);
    }
  }
  ...
  void _drawEagle(Canvas canvas, TreeFrame frame) {
    ...
    final ink = Paint()..color = palette.bark.withValues(alpha: 0.85);
    canvas.save();
    ...
    canvas.drawOval(Rect.fromCenter(center: Offset.zero, width: s * 1.1, height: s * 0.4), ink);
    canvas.drawCircle(Offset(s * 0.6, 0), s * 0.14, ink);
    canvas.restore();
  }
